...
export const PokeCard = ({ url, name }) => {
  ...
  async function fetchPokeDetailData() {
    try {
      ...
      const pokeData = formatPokeData(res.data);
      setPokemon(pokeData);
    } catch (e) {
      console.error(e);
    }
  ...
  return (
    <React.Fragment>
      {pokemon && (
        <Link
          to={`/pokemon/${name}`}
          className={`box-border rounded-lg ${border} w-[8.5rem] h-[8.5rem] z-0 bg-slate-800 items-center justify-between`}
        >
          <div
          ...
          <div
            className={`${bg} text-xs text-zinc-100 h-[1.5rem] rounded-b-lg uppercase font-medium flex justify-center pt-1`}
          >
            {pokemon.name}
          </div>
        </Link>
